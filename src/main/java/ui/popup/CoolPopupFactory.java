 ...
 * 
 * CoolPopupFactory.java at 2015-2-6 16:10:04, original version by Jack Jiang.
 * You can contact author with jb2011@163.com.
 */
package ui.popup;

import ui.widget.NinePatchBgPanel;
 ...
 * @version 1.0
 */
public class CoolPopupFactory extends PopupFactory
{
	/* (non-Javadoc)
...
//						isTooltip ? 1.0f : isComboBoxPopup?0.95f : 0.95f);//0.85f : 0.95f);//0.8f : 0.95f);
				...
				//图片填充背景的内容面板
				NinePatchBgPanel imageContentPane = new NinePatchBgPanel().setN9(
						isTooltip?
								NPIconFactory.getInstance().getTooltipBg()
								:isComboBoxPopup?//是下拉框列表则使一个跟JScrollPane一样的背景图，好看！
										NPIconFactory.getInstance().getScrollPaneBorderBg()
											:NPIconFactory.getInstance().getPopupBg()
				);
				...
				if(contents instanceof JComponent)
				{
					...
					((JComponent)contents).setBorder(
							isTooltip?
									BorderFactory.createEmptyBorder(107,27,12,27)//6,8,12,12)
